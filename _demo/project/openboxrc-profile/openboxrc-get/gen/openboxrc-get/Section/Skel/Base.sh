...
is_debug () {
	if [ "$DEBUG_OPENBOXRC_GET" = "true" ]; then
		return 0
	fi
...
base_var_init () {
	...
	if [ "none${THE_CMD_FILE_NAME}" = 'none' ]; then
		THE_CMD_FILE_NAME='openboxrc-get'
	fi
	...
	THE_SELF_UPDATE_URL='https://raw.githubusercontent.com/samwhelp/note-about-openbox/gh-pages/_demo/project/openboxrc-profile/openboxrc-get/openboxrc-get'
