...
mod_xephyr () {
	...
	mod_xephyr_run_openbox "$1" "$2"

}


mod_xephyr_run_openbox () {
	...
	if [ "none$config_file_path" = "none" ]; then
		util_error_echo "DISPLAY=:100 openbox"
		DISPLAY=:100 openbox
	else
		util_error_echo "DISPLAY=:100 openbox -c $config_file_path"
		DISPLAY=:100 openbox -c $config_file_path
	fi
